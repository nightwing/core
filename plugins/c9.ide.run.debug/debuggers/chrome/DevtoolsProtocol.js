...
function getSocket(options, callback) {
    ...
    function connectPort(cb) {
        app.net.connect(socketPath, {}, function(err, s) {
            ...
            socket.send(JSON.stringify({ $: "connect", port: options.port, host: options.host }));
...
(function() {
    ...
    this.setvariablevalue = function(name, value, scopeNumber, frameIndex, callback) {
        ...
        a ={
            ...
            "params": {
                "objectId": "{\"injectedScriptId\":1,\"id\":91}",
                "functionDeclaration": "function(a, b) { this[a] = b; }",
                "arguments": [
                    { "value": "filename" },
                    { "objectId": "{\"injectedScriptId\":1,\"id\":1125}" }
                ],
                "silent": true
            }
        }
